...
class ProcessEmbedENC(process_base.ProcessBase):
        ...
        for jetR in self.jetR_list:
            ...
            for observable in ['rho_local', 'mult']:
                for thrd in self.thrd_list:
                    ...
                    for coneR in self.coneR_list:
                        ...
                        # wta cone
                        name = 'h_wta_jetcone{}_matched_{}_JetPt_ch_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        # mbcone
                        name = 'h_mbcone{}_matched_{}_JetPt_ch_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        # wta mbcone
                        name = 'h_wta_mbcone{}_matched_{}_JetPt_ch_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        # wta perpcone
                        name = 'h_wta_perpcone{}_matched_{}_JetPt_ch_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        # wta cone combined
                        name = 'h_wta_jetcone{}_matched_{}_JetPt_ch_combined_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, comb jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        # mbcone combined
                        name = 'h_mbcone{}_matched_{}_JetPt_ch_combined_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        # wta mbcone combined
                        name = 'h_wta_mbcone{}_matched_{}_JetPt_ch_combined_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle(observable)
                        ...
                        name = 'h_wta_perpcone{}_matched_{}_JetPt_ch_combined_R{}_{}'.format(coneR, observable, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, obs_nbins, obs_bins)
                        h.GetXaxis().SetTitle('p_{T, comb jet}')
                        h.GetYaxis().SetTitle(observable)
            ...
            for thrd in self.thrd_list:
                ...
                for pair_type_label in self.pair_type_labels:
                    ...
                    name = 'h_jetcone_matched_ENC{}_JetPt_ch_R{}_{}'.format(str(ipoint)+pair_type_label, R_label, thrd_label)
                    print('Initialize histogram',name)
                    h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                    h.GetXaxis().SetTitle('p_{T, pp jet}')
                    h.GetYaxis().SetTitle('R_{L}')
                    ...
                    name = 'h_jetcone_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(str(ipoint)+pair_type_label, R_label, thrd_label)
                    print('Initialize histogram',name)
                    h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                    h.GetXaxis().SetTitle('p_{T, comb jet}')
                    h.GetYaxis().SetTitle('R_{L}')
                    ...
                    name = 'h_wta_jetcone_matched_ENC{}_JetPt_ch_R{}_{}'.format(str(ipoint)+pair_type_label, R_label, thrd_label)
                    print('Initialize histogram',name)
                    h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                    h.GetXaxis().SetTitle('p_{T, pp jet}')
                    h.GetYaxis().SetTitle('R_{L}')
                    ...
                    name = 'h_wta_jetcone_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(str(ipoint)+pair_type_label, R_label, thrd_label)
                    print('Initialize histogram',name)
                    h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                    h.GetXaxis().SetTitle('p_{T, comb jet}')
                    h.GetYaxis().SetTitle('R_{L}')
                    ...
                    for coneR in self.coneR_list:
                        ...
                        # jet mbcone
                        name = 'h_jet_mbcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle('R_{L}')
                        ...
                        # jet mbcone combined
                        name = 'h_jet_mbcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                        h.GetXaxis().SetTitle('p_{T, comb jet}')
                        h.GetYaxis().SetTitle('R_{L}')
                        ...
                        # jetcone mbcone
                        name = 'h_jetcone_mbcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle('R_{L}')
                        ...
                        name = 'h_jetcone_mbcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                        h.GetXaxis().SetTitle('p_{T, comb jet}')
                        h.GetYaxis().SetTitle('R_{L}')
                        ...
                        # wta jetcone mbcone
                        name = 'h_wta_jetcone_mbcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                        h.GetXaxis().SetTitle('p_{T, pp jet}')
                        h.GetYaxis().SetTitle('R_{L}')
                        ...
                        name = 'h_wta_jetcone_mbcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                        print('Initialize histogram',name)
                        h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                        h.GetXaxis().SetTitle('p_{T, comb jet}')
                        h.GetYaxis().SetTitle('R_{L}')
                        ...
                        if (pair_type_label == '_sb'):
                            name = 'h_2perpcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_wta_2perpcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_2perpcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_wta_2perpcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_2mbcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_wta_2mbcone{}_matched_ENC{}_JetPt_ch_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_2mbcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
                            ...
                            name = 'h_wta_2mbcone{}_matched_ENC{}_JetPt_ch_combined_R{}_{}'.format(coneR, str(ipoint)+pair_type_label, R_label, thrd_label)
                            print('Initialize histogram',name)
                            h = ROOT.TH2D(name, name, 200, pt_bins, 50, RL_bins)
                            h.GetXaxis().SetTitle('p_{T, pp jet}')
                            h.GetYaxis().SetTitle('R_{L}')
    ...
    def analyze_events(self):
        ...
        while (iev_mc < self.nEvents_mc):
            ...
            self.parts_pythia_ch = self.df_fjparticles_mc.iloc[iev_mc]
            ...
            # only include particles from leading pp jets for now
            self.parts_pythia_ch_jet = jets_pp[0].constituents() 
            ...
            # read in another ME
            self.fj_particles_combined_beforeCS_mb2 = self.df_fjparticles.iloc[me2_iev]
    ...
    def analyze_one_jet(self):
        ...
        for i in range(0, len(jets_combined)):
            ...
            if ((pt_sub > highest_pt_sub) & (jets_combined[i].area() > 0.6*np.pi*self.jetR_list[0]*self.jetR_list[0])):
        ...
        jet_combined = jets_combined[highest_index]
        # mc_fraction = self.mc_fraction(jet_pp, jet_combined)
        # if ((mc_fraction < self.mc_fraction_threshold) or ((self.is_geo_matched(jet_combined, jet_pp, self.jetR_list[0]))==False)):
        #     return
        ...
        #-------------------------------------------------------------
        # fill ENC histograms
        # Total ENC for the one pp jet and all combined jets
        hname = 'h_ENC{{}}_JetPt_ch_R{}_{{}}'.format(R_label)
        self.fill_jet_histograms(hname, jet_pp)
        for j in jets_combined:
            hname = 'h_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(R_label)
            self.fill_jet_histograms(hname, j)
        # Only fill matched jets: one pp jet and one combined jet    
        self.fill_matched_jets(jet_combined, jet_pp, self.jetR_list[0])
        self.fill_matched_jetcone(jet_combined, jet_combined_wta, jet_pp, self.jetR_list[0])
        for coneR in self.coneR_list:
            self.fill_matched_perpcone(jet_combined, jet_combined_wta, jet_pp, self.jetR_list[0], coneR)
            self.fill_matched_2perpcone(jet_combined, jet_combined_wta, jet_pp, self.jetR_list[0], coneR)
            self.fill_matched_mbcone(jet_combined, jet_combined_wta, jet_pp, self.jetR_list[0], coneR)
            self.fill_matched_2mbcone(jet_combined, jet_combined_wta, jet_pp, self.jetR_list[0], coneR)
    ...
    def fill_matched_jets(self, jet_combined, jet_pp, jetR):
        ...
        if self.debug_level > 0:
            print('area',jet_combined.area(),'rho',self.rho,'matched pp jet pt',jet_pp.perp())
    ...
    def fill_matched_perpcone(self, jet_combined, jet_combined_wta, jet_pp, jetR, coneR):
        ...
        for mode in ['jetcone']:
            ...
            # fill rho local for matched comb jet using pp jet for jet pT
            hname = 'h_wta_perpcone{}_matched_{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
            self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_cone3)

            hname = 'h_wta_perpcone{}_matched_{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
            self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_cone3)
            
    def fill_matched_2perpcone(self, jet_combined, jet_combined_wta, jet_pp, jetR, coneR):
        ...
        # fill EEC for matched comb jet using pp jet for jet pT
        hname = 'h_2perpcone{}_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone)

        hname = 'h_2perpcone{}_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone)
        ...
        # fill EEC for matched comb jet using pp jet for jet pT
        hname = 'h_wta_2perpcone{}_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone) # Use the original E-scheme jet pT

        hname = 'h_wta_2perpcone{}_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone) # Use the original E-scheme jet pT

    #---------------------------------------------------------------
    # Fill mb cone for matched combined jets
    #---------------------------------------------------------------
    def fill_matched_mbcone(self, jet_combined, jet_combined_wta, jet_pp, jetR, coneR):
        ...
        for mode in ['jet','jetcone']:
            ...
            # fill EEC for matched comb jet using pp jet for jet pT
            hname = 'h_{}_mbcone{}_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(mode, coneR, R_label)
            self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone1)
            # self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone2)

            # fill EEC for matched comb jet using comb jet (rho subtracted) for jet pT
            hname = 'h_{}_mbcone{}_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(mode, coneR, R_label)
            self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone1)
            # self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone2)

            if (mode == 'jetcone'):
                # fill rho local for matched comb jet using pp jet for jet pT
                hname = 'h_mbcone{}_matched_{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
                self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_cone1)

                hname = 'h_mbcone{}_matched_{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
                self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_cone1)
        ...
        for mode in ['jetcone']:
            ...
            # fill EEC for matched comb jet using pp jet for jet pT
            hname = 'h_wta_{}_mbcone{}_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(mode, coneR, R_label)
            self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone3) # Use the original E-scheme jet pT

            # fill EEC for matched comb jet using comb jet (rho subtracted) for jet pT
            hname = 'h_wta_{}_mbcone{}_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(mode, coneR, R_label)
            self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone3)

            # fill rho local for matched comb jet using pp jet for jet pT
            hname = 'h_wta_mbcone{}_matched_{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
            self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_cone3)

            hname = 'h_wta_mbcone{}_matched_{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
            self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_cone3)

    def fill_matched_2mbcone(self, jet_combined, jet_combined_wta, jet_pp, jetR, coneR):
        ...
        # fill EEC for matched comb jet using pp jet for jet pT
        hname = 'h_2mbcone{}_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone)

        hname = 'h_2mbcone{}_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone)
        ...
        # fill EEC for matched comb jet using pp jet for jet pT
        hname = 'h_wta_2mbcone{}_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone) # Use the original E-scheme jet pT

        hname = 'h_wta_2mbcone{}_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_cone) # Use the original E-scheme jet pT
    
    #---------------------------------------------------------------
    # Fill jet cone for matched combined jets
    #---------------------------------------------------------------
    def fill_matched_jetcone(self, jet_combined, jet_combined_wta, jet_pp, jetR):
        ...
        # fill EEC for jet cones
        hname = 'h_jetcone_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_jetcone)

        hname = 'h_jetcone_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_jetcone)

        # fill rho local for wta jet cones
        hname = 'h_wta_jetcone{}_matched_{{}}_JetPt_ch_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_wta_jetcone) # Use the original E-scheme jet pT

        hname = 'h_jetcone{}_matched_{{}}_JetPt_ch_combined_R{}_{{}}'.format(coneR, R_label)
        self.fill_matched_rho_local_histograms(hname, jet_pp, jet_combined, coneR, parts_in_wta_jetcone)

        # fill EEC for wta jet cones
        hname = 'h_wta_jetcone_matched_ENC{{}}_JetPt_ch_R{}_{{}}'.format(R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_wta_jetcone)

        hname = 'h_wta_jetcone_matched_ENC{{}}_JetPt_ch_combined_R{}_{{}}'.format(R_label)
        self.fill_matched_ENC_histograms(hname, jet_pp, jet_combined, parts_in_wta_jetcone)
    ...
    def fill_matched_ENC_histograms(self, hname, jet_pp, jet_combined, cone_parts):
        ...
        if cone_parts == None:
            ...
            # print("In fill_matched_ENC_histograms...len(jet_combined.constituents()):", len(constituents))
        else:
            constituents = fj.sorted_by_pt(cone_parts)
            # print("In fill_matched_ENC_histograms...len(cone_parts):", len(constituents))
