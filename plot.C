...
void addLegendInfo(TLegend *l, TString ptbin, bool D0recon)
{
    l->SetTextSize(0.045);
    l->AddEntry("NULL", "PYTHIA8 pp + thermal", "h");
...
void plot()
{
    ...
    // string one("");
    const char infile[] = "/global/cfs/cdirs/alice/youqi/AnalysisResults.root";
    ...
    for (int i = 0; i < n_bins; i++)
    {
        ...
        for (std::string jetR : jetR_list)
        {
            ...
            {
                const std::string h1_name = "h_matched_dR_" + jetaxis_names[iobs] + "_combined_std_R04_" + pt_min + "pT" + pt_max;
                const std::string h2_name = "h_matched_dR_" + jetaxis_names[iobs] + "_combined_wta_R04_" + pt_min + "pT" + pt_max;
            ...
            std::vector<double> numjets_1;
            std::vector<double> numjets_2;

            for (int iobs = 0; iobs < numjetaxes; iobs++)
            {
                numjets_1.push_back(h1[iobs]->Integral());
                numjets_2.push_back(h2[iobs]->Integral());
                cout << "integrals: " << h1[iobs]->Integral() << ", " << h2[iobs]->Integral() << endl;
            }
            ...
            for (int iobs = 0; iobs < numjetaxes; iobs++)
            {
                ...
                { // no D0 reconstruction
                    ...
                    if (iobs == 0)
                    {
                        h1[iobs]->GetXaxis()->SetTitle("#DeltaR(combined#minuspp STD)");
                    }
                    else if (iobs == 1)
                    {
                        h1[iobs]->GetXaxis()->SetTitle("#DeltaR(combined#minuspp WTA)");
                    }
                }
